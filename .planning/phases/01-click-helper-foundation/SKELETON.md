...
- 웹스토어 비공개 배포·설치 화면 권한 안내·회사 시스템 클릭 확인(Phase 2)
...
- 01-03: 떨림 재입력이 한 번으로 줄고, 입력칸에서는 글자가, 밖에서는 도우미 키가 동작하며 모드 표시가 바뀐다
- 01-04: 커서를 대충 가져가면 가장 가까운 요소가 잡히고 클릭·스페이스바로 눌린다
- 01-05: 번호표를 켜서 숫자 키로 누른다
- 01-06: iframe 안 요소에도 번호가 겹침 없이 붙는다
- 01-07: 위험한 버튼은 끌려오지 않고, 번호로 고르면 빨간 확인 화면에서 Enter로만 누른다
- 01-08: 머무르기 클릭 / 01-09: 끌어서 놓기 두 번 누르기
- 01-10: 대신 누르기 한계(연습 사이트) 스파이크 기록
- 01-11: 사이트별 끄기·"도울 수 없음" / 01-12: 형식 변환 실패 보존·옛 도우미 정리
- 01-13: 5,000요소 50ms 측정·확대 무관 크기·서체·독립 DOM 감사

이후 phase는 이 뼈대의 결정(저장 규칙·메시지·프레임 분담·오버레이 규칙)을 바꾸지 않고 조각을 더한다:

- Phase 2: 중간 이용자 시험 + 웹스토어 비공개 배포 + 회사 시스템 클릭 확인
