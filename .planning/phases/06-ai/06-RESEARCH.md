...
- 한 달 사용 한도의 단위와 기본값 — 횟수 vs 금액. 기본 제안: 횟수. 2026-09-24 사용자 확정: 기본 제안대로 횟수(보낸 요청 수).
- 저장된 AI 번호를 언제 쓰는지 — 기본 제안: "화면 정리"를 다시 누를 때만 저장된 번호를 쓴다. 2026-09-24 사용자 확정: 기본 제안대로.
...
3. **캐시·한도의 정확한 정책(discretion 항목 다수)** (RESOLVED)
   ...
   - **RESOLVED:** 06-06 "사용자 확정 선택"이 기본 제안으로 정했고, 사용자가 2026-09-24에 그대로 확정했다("전부 추천대로 해") — 한도 단위는 보낸 요청 수, 저장된 번호는 "화면 정리" 카드를 다시 누를 때만 쓴다. 저장 열쇠·낡음 판정은 06-06 Task 1, 사용량을 세는 시점은 06-06 Task 2, 취소한 요청의 사용량은 06-07 Task 1이 맡는다. 나중에 결정을 다시 바꾸면 바꿀 자리가 06-06의 그 절에 적혀 있다.
